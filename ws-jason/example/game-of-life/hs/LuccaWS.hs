...
-- para ter um servidor na porta P
...
getPercepts p name isUpdate = do
    (Perceptions gp lp ua) <- readIORef p
    -- r eh a uniao das percepcoes de gp com as percepcoes (segundo elemento) dos dados locais
...
clearPerceptsLocal p name = do
    ...
    if (null gp) then
        ...
        let lp' = lp `minus` [(name, perception) | (name, perception) <- lp]
        modifyIORef p $ \(Perceptions _ lp _) -> (Perceptions [] lp [])
        return True
